...
analyse_blast <- function(x){
  ...
  results_pred$Predicted_label <- final_preds$Host_label
...
LOO_blast_preds <- do.call(rbind,lapply(directories,analyse_blast))

write_csv(LOO_blast_preds,"loo_blast_host_predict_results.csv")
...
node_res <- left_join(node_names,LOO_blast_preds)
...
png(filename="LOO_blast_results_graph.png",width=2000,heigh=4000,units="px")
...
png(filename="LOO_blast_results_graph_labelled.png",width=2000,heigh=4000,units="px")
