...
lookup_tbl <- read_csv("/Users/jamieherzig/Documents/clean_run/3_host_assignment/virus_host_lookup.csv")
...
write_csv(results_pred,"blast_pred_results_noskewcorrection.csv")

# calculate accuracy
accuracy <- results_pred %>% 
  count(Outcome) 

accuracy <- accuracy$n[1]/sum(accuracy$n)
...
myshape <- shapes[factor(node_res$Outcome,levels = c("Correct","Incorrect"))]
...
view(node_res %>% filter(Outcome == "Incorrect"))
