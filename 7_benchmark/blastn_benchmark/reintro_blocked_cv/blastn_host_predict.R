...
analyse_blast <- function(x){
  ...
  # bind metadata into the blast hits
  ...
  results_pred$subgraph <- str_extract(x,pattern = "\\d+")
  # write results
  write_csv(results_pred,paste0(x,"/results.csv"))

}

cat_all_results <- function(reintro){

  directories <- list.files(path = paste0(reintro),pattern = "subgraph*")
  
  setwd(reintro)
  
  reintro_res <- do.call(rbind,lapply(directories,analyse_blast))
  reintro_res$n <- str_extract_all(reintro,"\\d")[[1]][1]
  reintro_res$iter <- str_extract_all(reintro,"\\d")[[1]][2]
  
  write_csv(reintro_res,"reintro_results.csv")
...
reintro_all_res <- do.call(rbind,lapply(dirlist,cat_all_results))

# clean up names and calculate metrics for plotting
colnames(reintro_all_res) <- c("Accession","Organism_Name","Host_label","Predicted_label","result","Subgraph","N_samples","iter")

reintro_acc <- reintro_all_res %>% 
  count(Subgraph,N_samples,result,iter)

reintro_acc %<>% 
  group_by(Subgraph,N_samples,iter) %>% 
  mutate(total = sum(n)) %>% 
  pivot_wider(names_from = result, values_from = n)

colnames(reintro_acc) <- c( "Subgraph","N_samples","iter","total","Incorrect","Correct" )
...
# take average and SD of runs for each N sample over all iterations
reintro_acc %<>% 
  group_by(Subgraph,N_samples) %>% 
  dplyr::summarise(mean_accuracy = mean(Accuracy),sd_accuracy = sd(Accuracy))

facet_titles <- labeller(Subgraph = 
                           c("1" = "Mammalian rubulaviruses",
                             "2" = "Mammalian respiroviruses & fish viruses",
                             "3" = "Mammalian morbilliviruses",
                             "4" = "Mammalian jeilongviruses",
                             "6" = "Mammalian narmoviruses",
                             "8" = "Avian metaavulaviruses",
                             "9" = "Avian orthoavulaviruses",
                             "10" = "Mammalian henipaviruses"))

reintro_acc$Subgraph <- factor(reintro_acc$Subgraph,levels = c("1","2","3","4","6","8","9","10"))

write_csv(reintro_acc,"blast_reintro_accuracy_metrics.csv")

ggplot(reintro_acc,aes(x = N_samples, y = mean_accuracy,group = Subgraph)) +
  # geom_point(size = 2) +
  geom_line()+
  geom_pointrange(data = reintro_acc,
