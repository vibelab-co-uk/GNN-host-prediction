...
resample_blocks = readdir("../../../../6_GNN/1_generate_validation_sets/", join=true)[occursin.(rx_blocks, readdir("../../../../6_GNN/1_generate_validation_sets/", join=true))]
...
for block in resample_blocks
    ...
    function feature_comp_train(in_feats,out_dir,nhidden,nlayers)
        ...
        cv_out.reintroductions .= resample_iter[3] # add these as ID columns and to the results file name
        cv_out.iteration .= resample_iter[4]

        CSV.write(joinpath(out_dir,"nn_output_reintro_$(resample_iter[3])_iter_$(resample_iter[4]).csv"),cv_out) # write the full ablation prediction results table
    end
