...
ggplot(agg_results %>% dplyr::filter(tax_level == "Order"),aes(x = Members, y = mean_accuracy)) +
  ...
  theme(strip.background = element_rect(fill="white"),
        ...
        legend.position = "top",
        plot.margin = margin(0,2.5,0,0,unit = "cm")) +
...
ggsave("./results_summary/single_ablation_simple_feats_order_byparams_nonoinfo.png",width = 2500,height=2500,units="px")
...
for(i in 1:nrow(bt_epochs)){
  ...
  if(grepl("rand",bt_epochs$feat[i]) == T){
    ...
    bt_output <- read_csv(paste0("./feature_sets/results_",bt_epochs$feat[i],"/abl_output_",bt_tbl$nhidden[rand_index],"_",bt_tbl$nlayers[rand_index],".csv"))
  } else {bt_output <- read_csv(paste0("./feature_sets/results_",bt_tbl$feat[i],"/abl_output_",bt_tbl$nhidden[i],"_",bt_tbl$nlayers[i],".csv"))}
...
plot_tbl <- agg_results %>% 
  filter(feat == "aaconly" & tax_level == "Order" & nhidden == aac_bt$nhidden & nlayers == aac_bt$nlayers |
          feat == "dintonly" & tax_level == "Order" & nhidden == dint_bt$nhidden & nlayers == dint_bt$nlayers |
           feat == "dponly" & tax_level == "Order" & nhidden == dp_bt$nhidden & nlayers == dp_bt$nlayers |
