...
ggplot(agg_results %>% dplyr::filter(tax_level == "Class"),aes(x = Members, y = mean_accuracy)) +
  geom_point(aes(col = Ablation),position = "jitter",size = 2) +
  geom_point(data = noinfo_class,aes(x = Members, y = mean_accuracy),col="grey3",shape = 4) +
  scale_color_manual(values = palette_OkabeIto_6) + 
  theme_bw() +
  scale_y_continuous(breaks = seq(0,1,0.1))+
  guides(col = "none") +
...
ggsave("single_ablation_simple_feats_class.png",width = 2500,height=2500,units="px")
