...
ablation_analysis <- function(abl_tbl,subgraph){
  ...
  if(length(unique(full_roc_tbl$obs)) == 1 & length(unique(full_roc_tbl$Group) == 1)){
    if(unique(full_roc_tbl$obs) == "Positive"){
      full_out <- tibble(Metric = c("SENS", "SPEC", "MCC", "Informedness", "PREC", "NPV", "FPR", "F1", "TP", "FP",          
                         ...
                         Taxa = subgraph_tax, Ablation = abl_tbl$drop_feat[1],Subgraph = subgraph)
    } else {
      full_out <- tibble(Metric = c("SENS", "SPEC", "MCC", "Informedness", "PREC", "NPV", "FPR", "F1", "TP", "FP",          
                                    "TN", "FN", "AUC-ROC", "AUC-PR", "AUC-PRG"),
    ...
    message(paste0("Subgraph ",subgraph,"; ",abl_tbl$drop_feat[1]," ablated; some metrics could not be calculated due to small sample size"))
  } else {
    ...
    full_out$Ablation <- abl_tbl$drop_feat[1]
    full_out$Subgraph <- subgraph
    
    full_out %<>%
      dplyr::select("Metric","Score","CI","Taxa","Ablation","Subgraph")
    
    message(paste0("Subgraph ",subgraph,"; ",abl_tbl$drop_feat[1]," ablated processed succesfully"))
  }
  ...
  agg_no_info$Ablation <- abl_tbl$drop_feat[1]
...
write_res <- function(results_dir){
  ...
  result_preds <- read_csv("abl_output.csv")
  ...
  results_list <- result_preds %>% 
    group_by(drop_feat) %>% 
    group_split()
  
  # apply analysis function across all ablated features and subgraphs
  abl_out <- cmapply(abl_tbl = results_list, subgraph = c(0,unique(subgraph_membership$Subgraph)), ablation_analysis)
...
cat_agg <- function(results_dir,hidden,model){
  setwd(results_dir)
...
cat_roc <- function(results_dir,hidden,model){
  setwd(results_dir)
...
cat_noinfo <- function(results_dir,hidden,model){
  setwd(results_dir)
