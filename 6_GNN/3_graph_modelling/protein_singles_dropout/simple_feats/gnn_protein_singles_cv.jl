...
include("./gnn_funcs_ablation.jl") # load loss and train functions
...
# define a tuple with the names of all proteins
prot_names = ["Genomic","Nucleocapsid","Matrix","Fusion","Attachment","Polymerase"]
...
Base.@kwdef mutable struct Args
    ...
    pdropout = 0.3     # proportion dropout 
end
...
function feature_comp_train(in_feats,out_dir,nhidden,nlayers)
    ...
    # set up k-fold cross validation loop
        ...
        mask = BitVector(undef, nrow(node_names)) # create an empty training mask
        mask .= true
        ...
        # ablate protein features before we add the features to the graph
        for f in prot_names
            if nrow(select(node_feats,Regex("$f"))) == 0 # check if feature data exists for this protein; if not, move to the next iteration
                # note use of literal verbose Regex form instead r"" is necessary to interpolate the variable in each loop iteration
                continue
            else
                abl_feats = select(node_feats, :Node,:Accession,Regex("$f"))
            
                # convert the dataframe to an array with each column as feature vector for a node and assign node features
                g.ndata.features = transpose(Matrix{Float32}(abl_feats[:, 3:end]))

                ### train models
                abl_out = abl_train(g, out_dir, nhidden = nhidden, nlayers = nlayers)
                abl_out.drop_feat .= f # add a new column reporting the feature used
    ...
    for prot in prot_names
        if nrow(select(node_feats,Regex("$prot"))) > 0 # we again need to check how many proteins have feature data available
            i += 1
        end
    end

    # finally add a column reporting which cv fold results came from
    cv_out.cv_fold = repeat(1:ncol(cv_folds), inner = nrow(node_feats) * i)
    CSV.write(joinpath(out_dir,"abl_output.csv"),cv_out) # write the full ablation prediction results table
end
