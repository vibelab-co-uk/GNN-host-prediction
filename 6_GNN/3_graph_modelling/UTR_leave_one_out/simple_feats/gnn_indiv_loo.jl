...
best_tune = CSV.File("../../../../6_GNN/3_graph_modelling/UTR_kfold_cv/simple_feats/results_summary/best_tune_acc_allUTR.csv") |> DataFrame

# sort to match input feats/out directories (alphabetical)
sort!(best_tune, [:condition,:feat])
...
# edgelist
coo_edgelist = readdlm("../../../../5_phylogeny/3_generate_graph/graph_data/patristic_paraymxo_graph_UTR.edgelist", Int64)
...
# edge weights
edge_weights = CSV.File("../../../../5_phylogeny/3_generate_graph/graph_data/patristic_paramyxo_graph_branch_weights_UTR.csv") |> DataFrame

# node metadata
node_names = CSV.File("../../../../5_phylogeny/3_generate_graph/graph_data/patristic_paramyxo_graph_node_names_UTR.csv") |> DataFrame
rename!(node_names, ["Node", "Accession"]) 

# host labels
host_labels = CSV.File("../../../../3_host_assignment/all_cluster_hosts_UTR.csv") |> DataFrame
...
function feature_comp_train(in_feats,out_dir,nhidden,nlayers)
    ...
    loo_out = DataFrame()# create empty DF to fill with results

    # set up loop over individual nodes
    for node in node_names.Node
        ...
        mask[node] = false # assign the prediction mask from the current prediction node

        # use the mask to assign the prediction node
        ...
        append!(loo_out,abl_out)
    end
    
    CSV.write(joinpath(out_dir,"loo_output.csv"),loo_out) # write the full ablation prediction results table
end
