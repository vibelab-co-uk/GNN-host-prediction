...
write_res <- function(results_dir,nhidden,nlayers){
  setwd(results_dir)
  ...
  result_preds <- read_csv(paste0("abl_output_",nhidden,"_",nlayers,".csv"))
  ...
  write_csv(agg_results_full,paste0("aggregate_results_",result_preds$Hidden_layers[1],"_",result_preds$Model_structure[1],".csv"))
  write_csv(ROC_results_full,paste0("ROC_PR_results_",result_preds$Hidden_layers[1],"_",result_preds$Model_structure[1],".csv"))
  write_csv(noinfo_results_full,paste0("no_info_results_",result_preds$Hidden_layers[1],"_",result_preds$Model_structure[1],".csv"))
  
  setwd("../..")
}

cat_agg <- function(results_dir,nhidden,nlayers){
  setwd(results_dir)
  
  agg_results <- read_csv(paste0("aggregate_results_",nhidden,"_",nlayers,"convlayers_weighted.csv"))
  
  agg_results$feat <- str_split(str_split(results_dir,pattern = "/")[[1]][3],pattern = "_")[[1]][3]
  agg_results$params <- paste(nhidden,nlayers,sep = "_")
    
  setwd("../..")
  return(agg_results)
}

cat_roc <- function(results_dir,nhidden,nlayers){
  setwd(results_dir)
  
  full_out <- read_csv(paste0("ROC_PR_results_",nhidden,"_",nlayers,"convlayers_weighted.csv"))
  
  full_out$feat <- str_split(str_split(results_dir,pattern = "/")[[1]][3],pattern = "_")[[1]][3]
  full_out$params <- paste(nhidden,nlayers,sep = "_")
  
  setwd("../..")
  return(full_out)
}

cat_noinfo <- function(results_dir,nhidden,nlayers){
  setwd(results_dir)
  
  full_out <- read_csv(paste0("no_info_results_",nhidden,"_",nlayers,"convlayers_weighted.csv"))
