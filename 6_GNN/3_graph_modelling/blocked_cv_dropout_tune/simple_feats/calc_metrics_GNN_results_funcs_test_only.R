...
write_res <- function(results_dir,dropout){
  setwd(results_dir)
  
  result_preds <- read_csv(paste0("abl_output_dropout_",dropout,".csv"))
  ...
  abl_out <- cmapply(FUN = subgraph_analysis,subgraph = c(1,2,3,4,6,8,9,10),MoreArgs = list(abl_tbl = result_preds))
  ...
  write_csv(agg_results_full,paste0("aggregate_results_",dropout,"_testonly.csv"))
  write_csv(ROC_results_full,paste0("ROC_PR_results_",dropout,"_testonly.csv"))
  write_csv(noinfo_results_full,paste0("no_info_results_",dropout,"_testonly.csv"))
  
  setwd("../..")
}

cat_agg <- function(results_dir,dropout){
  setwd(results_dir)
  
  agg_results <- read_csv(paste0("aggregate_results_",dropout,"_testonly.csv"))
  
  agg_results$feat <- str_split(str_split(results_dir,pattern = "/")[[1]][3],pattern = "_")[[1]][2]
  agg_results$dropout <- dropout
  
  setwd("../..")
  return(agg_results)
}

cat_roc <- function(results_dir,dropout){
  setwd(results_dir)
  
  full_out <- read_csv(paste0("ROC_PR_results_",dropout,"_testonly.csv"))
  
  full_out$feat <- str_split(str_split(results_dir,pattern = "/")[[1]][3],pattern = "_")[[1]][2]
  full_out$dropout <- dropout
  
  setwd("../..")
  return(full_out)
}

cat_noinfo <- function(results_dir,dropout){
  setwd(results_dir)
  
  full_out <- read_csv(paste0("no_info_results_",dropout,"_testonly.csv"))
