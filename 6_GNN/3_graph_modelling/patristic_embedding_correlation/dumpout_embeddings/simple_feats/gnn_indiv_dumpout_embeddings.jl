...
include("./gnn_funcs_dumpout.jl") # load loss and train functions
...
function feature_comp_train(in_feats,out_dir,nhidden,nlayers)
    ...
    for c in 1:ncol(cv_folds)
        ...
        ### train models
        dumpout_out = dumpout_train(g, c, out_dir, nhidden = nhidden, nlayers = nlayers)
    
        append!(cv_out,dumpout_out)
    end
    ...
    CSV.write(joinpath(out_dir,"dumpout_output.csv"),cv_out) # write the full ablation prediction results table
end
