...
function dumpout_train(g, cv_fold, out_dir; kws...)
    args = Args(; kws...)
    ...
    function report(epoch)
        ...
        open(joinpath(out_dir,"training_log_$(args.nhidden)_$(args.nlayers)convlayers_weighted_cvfold$cv_fold.txt"),"a") do io
    ...
    ## TRAINING
    report(0)
    z = tsne(model(g,X) |> transpose,2) 
    CSV.write(joinpath(out_dir,"raw_embeddings_$(args.nhidden)_$(args.nlayers)_epoch0_cvfold$cv_fold.csv"),DataFrame(model(g, X),:auto),append = true)
    CSV.write(joinpath(out_dir,"tsne_$(args.nhidden)_$(args.nlayers)_epoch0_cvfold$cv_fold.csv"),DataFrame(z,:auto),append = true)

    ## assess untrained model
    final_out = DataFrame(Observed = g.labels, Prediction_score = onecold(model(g, X)), Predicted = Vector{String}(undef,length(g.labels)),
                        ...
                        Train = g.train_mask, Hidden_layers = nhidden, Model_structure = string(args.nlayers)*"convlayers_weighted", Accession = g.ndata.accessions, 
    ...
    for epoch in 1:(args.epochs)
        ...
        CSV.write(joinpath(out_dir,"raw_embeddings_$(args.nhidden)_$(args.nlayers)_epoch$(epoch)_cvfold$cv_fold.csv"),DataFrame(model(g, X),:auto),append = true)
