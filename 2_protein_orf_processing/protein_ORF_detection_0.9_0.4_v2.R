...
prot_refseqs <- readAAStringSet("paramyxoviridae_prot_refseqs.fasta")
...
refseqs_metadata <- read_csv("paramyxoviridae_prot_refseq_metadata.csv")
...
# manual corrections
jeilong_set %<>% 
  dplyr::mutate(Protein = case_when(seqnames == "PP990582.1" & start == 7113 ~ "Undefined", # two attachment proteins assigned, correct assignment determined by pairwise alignment to ref
                                    seqnames == "MW579602.1" & start == 6641 ~ "Fusion", # fusion not identified, correct assignment determined by pairwise alignment to ref
                                    seqnames == "MG203876.1" & start == 10076 ~ "Attachment", # attachment not identified, correct assignment determined by pairwise alignment to ref
                                    seqnames == "AY900001.1" & start == 10026 ~ "Undefined", # two attachment proteins assigned, correct assignment determined by pairwise alignment to ref
                                    seqnames == "MZ328288.1" & start == 7086 ~ "Undefined", # two attachment proteins assigned, correct assignment determined by pairwise alignment to ref
                                    seqnames == "PP990581.1" & start == 7113 ~ "Undefined", # two attachment proteins assigned, correct assignment determined by pairwise alignment to ref
                                    .default = Protein))
...
# manual corrections
big_set %<>% 
  dplyr::mutate(Protein = case_when(seqnames == "LC187306.1" & start == 4778 ~ "Matrix", # several candidates for matrix protein, correct assignment determined by pairwise alignment to ref
                                    seqnames == "LC431581.1" & start == 2828 ~ "Matrix", # matrix protein not ID'd, correct assignment determined by pairwise alignment to ref
                                    seqnames == "LC431581.1" & start == 155 ~ "Nucleocapsid", # nucleocapsid misassigned, correct assignment determined by pairwise alignment to reference
                                    seqnames == "LC431581.1" & start == 1235 ~ "Undefined", #
                                    seqnames == "OL630969.1" & start == 1694 ~ "Undefined", # two nucleocapsid assigned, correct assignment determined by pairwise alignment to bat paramyxovirus virus ref
                                    seqnames == "OL630969.1" & start == 7367 ~ "Undefined", # two fusion assigned, correct assignment determined by pairwise alignment to bat paramyxovirus virus ref
                                    seqnames == "EU267273.1" & start == 4540 ~ "Undefined", # two matrix assigned, correct assignment determined by pairwise alignment to ref
                                    seqnames == "LC187308.1" & start == 1687 ~ "Undefined", # two nucleocapsid assigned, correct assignment determined by pairwise alignment to ref
                                    seqnames == "LC187310.1" & start == 1693 ~ "Undefined", # two nucleocapsid assigned, correct assignment determined by pairwise alignment to avian metaavulavirus 8
                                    seqnames == "OP236020.1" & start == 4414 ~ "Undefined", # two matrix assigned, correct assignment determined by pairwise alignment to ref
                                    seqnames == "OR286505.1" & start == 4461 ~ "Undefined", # two matrix assigned, correct assignment determined by pairwise alignment to ref (PdPR-V)
                                    seqnames == "OR367430.1" & start == 8408 ~ "Polymerase", # no polymerase assigned, correct assignment determined by pairwise alignment to ref
                                    seqnames == "PV102958.1" & start == 384 ~ "Nucleocapsid", # nucleocapsid misassigned, correct assignment determined by pairwise alignment to reference
                                    seqnames == "PV102958.1" & start == 1501 ~ "Undefined", #
                                    seqnames == "PV005834.1" & start == 7233 ~ "Undefined", # two fusion assigned, correct assignment determined by pairwise alignment to ref
                                    .default = Protein))
...
###############################
### WORKING ALIGNMENT
# current_seq <- cluster_centroids["PV005834.1"]
# 
# target_orfs <- big_set %>%
#   filter(seqnames == "PV005834.1")
# 
...
# 
# alignment <- pwalign::pairwiseAlignment(prot_refseqs[grepl("Bat paramyxovirus",names(prot_refseqs)) & grepl("Fusion",names(prot_refseqs))],prot_orfs[5])
# 
...
# assess UTRs
UTR_3 <- long_orfs_df %>% 
  filter(Protein == "Nucleocapsid") %>% 
  group_by(seqnames) %>% 
  summarise("putative_UTR" = start)

UTR_5 <- long_orfs_df %>% 
  filter(Protein == "Polymerase") %>% 
  group_by(seqnames) %>% 
  summarise("putative_UTR" = seq_length - end+1)
...
print(length(unique(validation_set$Organism_Name))) # 70 unique viruses for which we have a refseq available
print(length(unique(validation_set$seqnames))) # 132 unique sequences for which we have a refseq available
...
# set up empty stringsets and list
check_me <- AAStringSet()
bad_alignment_list <- vector("list",nrow(neg_scores))

# this unwieldly selection pulls out the relevant alignment for each negative scoring alignment in the neg_scores tbl and returns a list of each alignment
for(i in 1:nrow(neg_scores)){
  for(j in 1:length(validation_alignments[[neg_scores[i,]$Index]][[3]])){
    check_me <- AAStringSet(c(check_me,validation_alignments[[neg_scores[i,]$Index]][[3]][[j]][grepl(neg_scores[i,]$Protein,names(validation_alignments[[neg_scores[i,]$Index]][[3]][[j]]))]))
  }
  bad_alignment_list[i] <- check_me
  check_me <- AAStringSet()
}

# manually inspect low scoring alignments
names(bad_alignment_list[[1]]) # bat paramyxovirus attachment protein
# DECIPHER::BrowseSeqs(bad_alignment_list[[1]]) # the subject is truncated, but aligns well for ~300 AA of its length

names(bad_alignment_list[[2]]) # salmon aquaparamyxovirus nucleocapsid protein
# DECIPHER::BrowseSeqs(bad_alignment_list[[2]]) # does appear to align but large sequence divergence. Appears evolutionarily distant to the ref. Final ~100 AA do not align

names(bad_alignment_list[[3]]) # salmon aquaparamyxovirus attachment protein
# DECIPHER::BrowseSeqs(bad_alignment_list[[3]]) # closer alignment than the nucleocapsid, but still quite distant from reference

names(bad_alignment_list[[4]]) # bat paramyxovirus nucleocapsid protein
# DECIPHER::BrowseSeqs(bad_alignment_list[[4]]) # the subject is truncated, but aligns relatively well for ~400 AA of its length

names(bad_alignment_list[[5]]) # bat paramyxovirus fusion protein
# DECIPHER::BrowseSeqs(bad_alignment_list[[5]]) # quite distant from the reference, but is clearly a homologous protein

names(bad_alignment_list[[6]]) # bat paramyxovirus attachment protein
# DECIPHER::BrowseSeqs(bad_alignment_list[[6]]) # very distant from the reference, but does appear to be a homologous protein 

# all of these low scoring alignments are clearly homologues, albeit quite distant from the reference and/or truncated. ORF assignment is working well

# Write outputs -----------------------------------------------------------
# function to extract all ORFs from each genome sequence
...
# function to extract all leader/trailer UTRs from each genome sequence
extract_UTRs <- function(seq_id){
  ...
  leader <- mapply(subseq,current_seq,start=1,end=target_leader$putative_UTR)
  
  names(leader) <- paste(seq_id,"3prime_leader",sep="|")
  
  target_trailer <- UTR_5 %>% 
    filter(seqnames == names(current_seq))
  
  trailer <- mapply(subseq,current_seq,start=width(current_seq)-target_trailer$putative_UTR,end=width(current_seq))
  
  names(trailer) <- paste(seq_id,"5prime_trailer",sep="|")
  
  UTR_seqs <- c(DNAStringSet(trailer),DNAStringSet(leader))
  
  # return individual ORFs (for mafft alignment)
  return(UTR_seqs)
}

UTR_set <- do.call(c,lapply(unique(long_orfs_df$seqnames),extract_UTRs))

# split into individual UTRs
all_trailers <- UTR_set[grepl("5prime",names(UTR_set))]
all_leaders <- UTR_set[grepl("3prime",names(UTR_set))]

writeXStringSet(all_trailers,"5prime_trailer_UTRs.fasta")
writeXStringSet(all_leaders,"3prime_leader_UTRs.fasta")

hist(UTR_3$putative_UTR)
hist(UTR_5$putative_UTR,breaks = seq(0,2500,by = 100))
